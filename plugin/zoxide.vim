...
command! -nargs=? -complete=file Za call zoxide#exec([get(g:, 'zoxide_executable', 'zoxide'), 'add', <q-args>])
command! -nargs=? -complete=file Zr call zoxide#exec([get(g:, 'zoxide_executable', 'zoxide'), 'remove', <q-args>])
